...
# =============================================================================
NUM_ANTS = 90                    # m: Number of ants per iteration
NUM_ITERATIONS = 400             # Number of ACO iterations
...
def _generate_speeds_for_permutation(permutation, geom):
    """
    Generate speeds using the SAME logic as SA:
      - Leader gets a high-ish speed
      - Followers must NOT exceed predecessors
      - Global bounds enforced
    """
    v_min_global, v_max_global = config.velocity_range
    ...
    for approach, queue in geom.entry_queues.items():
        ...
        # Find the leader (first vehicle from this queue in permutation)
        leader = None
        for v in queue:
        ...
        # Leader gets high speed
        leader_speed = random.uniform(
            v_min_global + 0.5*(v_max_global - v_min_global),
            v_max_global
        )
        speeds_dict[leader.id] = leader_speed

        # Followers (in queue order)
        last_speed = leader_speed
        followers = [
            v for v in queue 
            if v.id != leader.id and v.id in [p.id for p in permutation]
        ]

        for v_f in followers:
            # Follower cannot exceed predecessor
            s_new = random.uniform(v_min_global, last_speed)
            speeds_dict[v_f.id] = s_new
            last_speed = s_new
    ...
    for v in permutation:
        if v.id not in speeds_dict:
            final_speeds.append(random.uniform(v_min_global, v_max_global))
        else:
            final_speeds.append(speeds_dict[v.id])

    # Final enforcement of constraints (same as SA)
    return validate_speeds(permutation, final_speeds, geom)
